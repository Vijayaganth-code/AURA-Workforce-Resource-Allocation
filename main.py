...
from services import dashboard_data,candidates_for,recommend,simulate,create_recommendation,approve_reallocation,answer_question
...
def event(body:EventRequest):
 with connection()as c:c.execute('INSERT INTO events VALUES (?,?,?,?,?,?)',(f'EV-{uuid.uuid4().hex[:10]}',body.event_type,body.employee_id,body.task_id,json.dumps(body.payload),datetime.now(timezone.utc).isoformat()))
...
def break_start(body:BreakRequest):
 with connection()as c:
  ...
  c.execute("UPDATE employees SET status='ON_BREAK',availability=0 WHERE employee_id=?",(body.employee_id,));c.execute('INSERT INTO break_sessions VALUES (?,?,?,?,?,?)',(f'B-{uuid.uuid4().hex[:10]}',body.employee_id,body.break_type,'ACTIVE',datetime.now(timezone.utc).isoformat(),None))
...
def leave_analyze(body:LeaveRequest):
 ...
 return simulate('leave',body.employee_id)
@app.post('/api/leave/request')
def leave_request(body:LeaveRequest):
 if body.end_date<body.start_date:raise HTTPException(422,'End date must not precede start date')
 with connection()as c:c.execute('INSERT INTO leave_requests VALUES (?,?,?,?,?)',(f'L-{uuid.uuid4().hex[:10]}',body.employee_id,body.start_date.isoformat(),body.end_date.isoformat(),'PENDING'))
